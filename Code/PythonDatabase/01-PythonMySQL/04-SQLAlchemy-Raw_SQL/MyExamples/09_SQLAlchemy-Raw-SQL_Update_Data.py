# 文件名：09_SQLAlchemy-Raw-SQL_Update_Data.py
# 功能说明：通过SQLAlchemy-Raw-SQL（MySQL+PyMySQL）修改数据库中的数据表的数据记录
...
#----------------------------------------------------------------------------
# 四、定义Raw SQL语句（更新数据记录）
#----------------------------------------------------------------------------
TABLE_NAME = 'mytable'
# SQL语句（更新数据表数据记录）
sql_str = "UPDATE {} set name = %s, url = %s WHERE name = %s".format(TABLE_NAME)
# 定义更新记录变量
val_data = ("Python", "https://www.python.org", "GitHub")
...
try:
    ...
    print('从数据表{}中成功更新'.format(TABLE_NAME), res_obj.rowcount, '条数据记录！')
except: # 如果更新数据记录失败，则给出失败提示
    print('从数据表{}中更新数据记录失败！'.format(TABLE_NAME))