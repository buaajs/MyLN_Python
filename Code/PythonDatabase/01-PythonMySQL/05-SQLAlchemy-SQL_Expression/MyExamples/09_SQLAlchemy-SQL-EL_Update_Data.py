# 文件名：09_SQLAlchemy-SQL_Expression_Update_Data.py
# 功能说明：通过SQLAlchemy-SQL_Expression（MySQL+PyMySQL）修改数据表中的数据记录
...
#----------------------------------------------------------------------------
# 四、构造修改数据SQL Expression
#----------------------------------------------------------------------------
sql_update = mytable.update().values(name='Python', url='https://www.python.org').where(mytable.c.name == 'GitHub')
...
#----------------------------------------------------------------------------
try: 
    myresult = myconn.execute(sql_update)
    # 修改数据记录成功给出提示
    print('向数据表中成功更新', myresult.rowcount, '条数据记录！')
except: # 如果修改数据记录失败，则给出失败提示
    print('向数据表中更新数据记录失败！')
