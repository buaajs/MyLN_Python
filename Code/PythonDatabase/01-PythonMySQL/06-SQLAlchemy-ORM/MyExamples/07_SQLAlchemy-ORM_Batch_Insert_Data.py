# 文件名：07_SQLAlchemy-ORM_Batch_Insert_Data.py
# 功能说明：通过SQLAlchemy-ORM（MySQL+PyMySQL）向数据表中批量插入数据记录
...
# 四、创建映射类实例对象
#-------------------------------------------------------------------------
mytable_obj2 = Mytable(name='Google', url='https://www.google.com')
mytable_obj3 = Mytable(name='GitHub', url='https://www.github.com')
mytable_obj4 = Mytable(name='Baidu',  url='https://www.baidu.com')
...
#-------------------------------------------------------------------------
try:
    # 通过add_all()方法向Session类实例对象mysession添加多个映射类实例对象
    mysession.add_all([mytable_obj2, mytable_obj3, mytable_obj4]) 
    # 提交到数据库执行
    mysession.commit()
    print('插入数据记录成功！')
