# 文件名：09_SQLAlchemy-ORM_Update_Data.py
# 功能说明：通过SQLAlchemy-ORM（MySQL+PyMySQL）修改数据表中的数据记录
...
try:
# 1. 创建查询对象query_obj 
    query_obj =  mysession.query(Mytable).filter_by(name = 'Github').first()
# 2. 对查询对象query_obj进行修改
    query_obj.name = 'Python'
    query_obj.url  = 'https://www.python.org'
# 3. 提交到数据库执行 
    mysession.commit()
    print('更新数据记录成功！')
